...
export function extractDomainName(url: string): string {
  try {
    ...
    // Remove common prefixes and get main domain
    const parts = hostname.split('.')
    const prefixes = ['www', 'docs', 'api', 'blog', 'help', 'support']
    ...
    return domainName.replace(/[^a-zA-Z0-9\-_]/g, '-').toLowerCase()
  } catch {
    return 'docs'
  }
}

export async function validateOutputDirectory(dir: string): Promise<boolean> {
  try {
